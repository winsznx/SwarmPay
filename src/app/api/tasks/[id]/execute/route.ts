...
export async function POST(
  ...
  try {
    ...
    const agent = agents.find((a: Agent) => a.id === task.assignedAgentId);
    ...
    if (isTaskComplex(task) && task.depth < 3 && !hasSubTasks) {
      ...
      await store.updateTask(task.id, { status: 'executing' }); // Mark parent as executing
    ...
    await store.distributePayment(agent.id, reward, task.id, `Manual Execution: ${(task as any).title || 'Task'}`);
    ...
    const subTasks = await store.getSubTasks(parentId);
